  ...
  const handleRarConvert = async (format: 'pdf' | 'cbz') => {
    ...
    try {
      ...
      if (blobs.length === 0) {
        ...
        dispatch({ type: "UPDATE_FILE_URL", payload: { id: top.id, url: top.virtualFile.objectUrl, extractedImages: imageUrls }});
        ...
        alert(`Conversion complete! Your ${ext.toUpperCase()} file is ready and downloading.`);
      }
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-lg text-white">RAR/CBR Detected</h3>
                <button onClick={handleSkipRar} className="text-white/50 hover:text-white transition-colors">
                  <X size={20} />
                </button>
              </div>
              <p className="text-white/70 text-sm mb-6">
                RAR and CBR files can be slow to view in the browser. Would you like to convert <strong>{rarQueue[0].file.name}</strong> to CBZ or PDF? It will immediately download once ready.
              </p>

              {isConvertingRar ? (
                <div className="flex flex-col items-center justify-center py-4 gap-3">
                  <Loader2 className="animate-spin text-forest-primary" size={32} />
                  <p className="text-sm text-forest-primary font-medium animate-pulse">Converting...</p>
                </div>
              ) : (
